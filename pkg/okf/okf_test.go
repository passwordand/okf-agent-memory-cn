...
func TestSearchSupportsChineseTerms(t *testing.T) {
	b := &okf.Bundle{Concepts: map[string]*okf.Concept{
		"auth/login": {
			ID:          "auth/login",
			Title:       "登录鉴权",
			Description: "登录鉴权采用 JWT，访问令牌过期后刷新。",
		},
		"memory/knowledge": {
			ID:          "memory/knowledge",
			Title:       "记忆库",
			Description: "长期记忆保存在知识库中。",
		},
	}, Graph: map[string][]string{}, InboundGraph: map[string][]string{}}

	for _, query := range []string{"登录", "鉴权", "记忆", "知识库", "JWT"} {
